...
7. **“沉淀 / 强调”必须更新协作约束，且写回本 skill（不是别处的记忆）**：当用户说要“沉淀”某项
   经验，或再次“强调”某个问题时，其意图默认是修改可复用的协作约束以避免以后再犯，而不是只修当前
   文档、口头确认、或写进对话 / 项目 / 用户级记忆。**在使用本 skill 的过程中被要求沉淀的，唯一权威
   落点就是本 `SKILL.md`（或其 `references/` 下的对应文件）**——把它写成一条可执行的硬约束或步骤，
   带上触发条件与行为要求，并回读验证。禁止用云端 memory（`context://memory/...`）或某个业务仓库的
   `project_memory.md` 来替代 skill 沉淀：那些是被动读取的记忆层，不会约束行为，属于填错地方。仅完成
   当前内容修改不算完成。若规则同时适用于姊妹 skill，必须同步其共享引用或副本，避免规则漂移。
8. **只处理「当前用户本人」的评论，别人的评论一律不落地**：划词协作的默认范围是**发起本次任务的
   用户自己写的评论**，不是文档里所有人的评论。文档常有多方 reviewer 的批注，其他人提的建议
   （哪怕很合理）都**不得**采纳进正文——那是文档主人自己的决策权。第 1 步就要拿到「当前用户」的
   身份标识（见下），第 3 步分类时把非本人的评论一律归为「不在范围、跳过」，既不改正文也不回
   `「ByAgent」`。仅当用户显式说「把 XX 的评论也处理了 / 所有评论都改」时才扩大范围。**采纳并
   写入他人评论 = 越权改稿，属翻车级错误。** 若已误改，立即 `block_delete` 回滚误加内容、恢复被
   连带删除的原块（用插入前快照 `content.xml` 取原文重插），并 `file.comment.replys delete`
   撤回误发的 `「ByAgent」` 回复。
9. **改动要给 reviewer 留可视差异（默认开启，用户说不用才关）**：编辑带评论 / 供他人 review 的
   飞书文档时，本轮改到的段落 / 框用黄色底色 `rgba(255,246,122,0.8)` 标注（正文改动包一层
   `<span background-color="rgba(255,246,122,0.8)">…</span>`；整块新增 / 重写用黄底
   `callout`）。目的：reviewer 不用逐段比对就能只看改动。标注本身也走就地改，`span` 只包变更
   文字、绝不删掉其它评论的 quote 锚点（对齐硬约束 2）。**文档顶部的变更记录不是默认动作**：
   一般只在用户完成方案评审后使用，而且必须由用户明确要求；未收到“补变更记录 / 维护 Change Log”
   等指令时，禁止自行创建或更新变更记录。
...
```
逐条记录：`quote`（锚点原文）、评论/回复意见、`is_solved`、**评论作者 `user_id`**。给每条编个号（[0][1]…）。

> **先确认「当前用户」是谁（对齐硬约束 8）。** 划词协作只处理**发起本次任务的用户本人**写的
> 评论。先拿到当前用户的 `open_id` / `user_id`（`lark-cli` 登录态即当前用户；必要时用
> `drive user` / `whoami` 类命令确认），再把每条评论的作者 `user_id` 与之比对。**非本人写的
> 评论一律标记「不在范围」，第 3 步直接跳过，既不改正文也不回 `「ByAgent」`。** 只有用户显式
> 要求「连 XX 的评论一起处理 / 所有评论都改」时才扩范围。
...
### 第 3 步 · 逐条对齐 + 分类
把每条评论的 quote 与当前正文比对，分三类：
- **非当前用户本人写的（不在范围，跳过）**：作者 `user_id` ≠ 当前用户 → 直接跳过，不改正文、
  不回 `「ByAgent」`（除非用户显式要求处理他人评论）。这是硬约束 8，优先于下面两类判断。
