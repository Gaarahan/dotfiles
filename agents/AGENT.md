...
- 当我处理完用户提出的“长任务”后，需要通过飞书给用户发一条通知（成功/失败都算）。
...
0) **用户显式要求触发（兜底）**
   - 用户明确提出“这是长任务 / 做完发飞书提醒 / 完成后通知我”等等类似表述时：无论步骤数、文件数、是否建立 TODO，都必须在收尾时发送 1 条飞书通知。

1) **TODO 触发（最优先）**
   - 本轮对话中我调用过 `TodoWrite` 并创建了 1 条及以上 todo。
   - 且在最终回复前，所有 todo 的状态都为 `completed`（没有 `pending`/`in_progress`）。
   - 说明：因此**不要为短任务随意创建 TODO**；一旦创建，就意味着必须发送飞书通知。

2) **多步骤触发**
   - 我在同一轮任务中完成了 3 个及以上“检查点”，且每个检查点都有可验证产物（例如：完成实现 / 跑通自检命令 / 产出结论报告）。
   - 反例：只执行了少量只读命令（如 `git diff`）用于查看上下文，不算检查点。

3) **多文件触发**
   - 我对仓库落盘修改涉及 3 个及以上文件，或跨 2 个及以上一级目录（例如同时改了 `agents/` + `bin/`）。

### 通知时机与次数

- 在任务“收尾输出”完成后发送（即：我准备结束本轮响应时）。
- **每个长任务只发 1 条**。
- 若任务失败/中断：也发 1 条“未完成/失败”通知，并在对话里说明失败原因与下一步。

### 通知内容（必须简短）

- 只发必要信息，推荐格式：`XXX 已完成，可查看结果` / `XXX 未完成（原因：YYY）`。
- 不要包含：密钥、token、内部链接、日志全文、diff 细节。

### 发送方式

- 优先使用本机命令 `feishu-im-send`（由用户环境提供 `FEISHU_APP_ID/FEISHU_APP_SECRET`，默认接收人可用 `FEISHU_RECEIVE_ID`）。
