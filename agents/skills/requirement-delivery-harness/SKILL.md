...
7. Re-read the latest published technical-solution revision immediately before coding and record that revision in task-local evidence. Do not implement from conversation memory, an earlier revision, or historical source behavior.
8. If summary text, matrices, examples, and detailed interface sections contradict each other, resolve the contradiction before coding. Do not silently combine incompatible fragments into a new contract.
9. Treat the approved technical solution as the implementation contract. Do not redesign approved entities, interfaces, states, or ownership boundaries merely because an upstream change is not yet present on the synchronized main branch.
10. When an approved upstream contract is missing, preserve the approved names and call shape with the smallest localized mock or adapter. Mark it with a TODO that names the upstream dependency and exact removal condition; keep the mock behavior within the shipped access and compatibility boundary.
11. Structure downstream code so the upstream contract can replace the mock without redesigning the consumer. Do not duplicate or permanently own another team's shared implementation.
12. Classify backend dependencies by delivery phase. When the backend is still under development and integration has not started, record the contract dependency but do not block frontend implementation on live-interface verification.
13. Defer live backend verification to the integration stage, and do not claim end-to-end acceptance before that verification succeeds.
